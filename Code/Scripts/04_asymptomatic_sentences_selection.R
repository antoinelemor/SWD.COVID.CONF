...
phrases_uniques <- SWD.conf_tokenised %>%
  ...
  summarise(sentence = first(sentence), 
            asymptomatic = first(asymptomatic),
            date = first(date), 
            .groups = 'drop') %>%
...
phrases_positives <- phrases_uniques %>% 
  filter(asymptomatic == 1)

# Fonction pour trouver les intervalles
trouver_intervalles <- function(df, id, sentence_id, range = 5) {
...
SWD.asymptomatic.sentences <- intervalles_df %>%
  group_by(doc_id) %>%
  do(traiter_groupe(.)) %>%
  ungroup()

# Exportation en CSV
write.csv(SWD.asymptomatic.sentences, file.path(export_path, "SWD.asymptomatic.sentences.csv"), row.names = FALSE)

# Sélectionner les 5 premières lignes
SWD.asymptomatic.sentences.sample <- head(SWD.asymptomatic.sentences, 25)

# Exporter les 5 premières lignes en CSV
write.csv(SWD.asymptomatic.sentences.sample, file.path(export_path, "SWD.asymptomatic.sentences.sample.csv"), row.names = FALSE)
